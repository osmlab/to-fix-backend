var queries = {};
queries.create_function = function(tableName, table_stats) {
	return 'CREATE OR REPLACE FUNCTION task_' + tableName + '(lockPeriod int , now int) '+
			...
			'BEGIN  '+
				...
				'IF result.key ISNULL THEN  '+
					...
					'SELECT count(*) INTO _skip FROM (SELECT attributes FROM ' + table_stats + ' WHERE attributes -> \'action\' = \'skip\'AND attributes ->\'key\' !=\'\')as sub;  '+
					'SELECT count(*) INTO _fix FROM (SELECT attributes FROM ' + table_stats + ' WHERE attributes -> \'action\' = \'fix\'AND attributes ->\'key\' !=\'\') as sub;  '+
					'SELECT count(*) INTO _noterror FROM (SELECT attributes FROM ' + table_stats + ' WHERE attributes -> \'action\' = \'noterror\' AND attributes ->\'key\' !=\'\') as sub;  '+
