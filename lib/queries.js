...
queries.create_function = function(tableName, table_stats) {
	return 'CREATE OR REPLACE FUNCTION task_' + tableName + '(lockPeriod int , now int) ' +
		...
		'result.key=\'complete\';  ' +
		'UPDATE task_details SET status = subquery.status FROM (SELECT CASE when MIN("time")=2147483647 THEN true  ELSE false END AS status FROM '+tableName+') AS subquery WHERE task_details.id=\''+tableName+'\'; '+
		'ELSE  ' +
		'UPDATE ' + tableName + '  SET time = (now+lockPeriod) WHERE key=result.key;' +
		'UPDATE task_details  SET updated = now WHERE id=\'' + tableName + '\';' +
		'END IF;  ' +
		'RETURN result;  ' +
		'END;  ' +
		'$$ LANGUAGE plpgsql;';
