...
module.exports = {
	count: function(client, request, reply) {
		var table = request.params.task.replace(/[^a-zA-Z]+/g, '').toLowerCase();
		//table = table.substring(0, table.length - 3); // We dont nne this line , this line work in time
		queue(1)
			.defer(function(cb) {
				// overall count
	...
	count_history: function(client, request, reply) {
		var table = request.params.task.replace(/[^a-zA-Z]+/g, '').toLowerCase();
	...
	track: function(client, request, reply) {
	...
	track_stats: function(client, request, reply) {
	...
	tasks: function(client, request, reply) {
		var query = 'SELECT id, title, source, tasktable, status FROM task_details ORDER BY status, title;'; // WHERE status = $1
		var tasks = [];
