...
module.exports = {
    ...
    csv: function(client, request, reply) {
        ...
        if (data.file) {
            ...
            var tableName = data.name.replace(/[^a-zA-Z]+/g, '').toLowerCase() + rs.alphaLower(3); //tableName will be the idtask
                            ...
                            setTimeout(function() {
                                queue(1)
                                    ...
                                    .defer(function(cb) {
                                        ...
                                        var query = queries.create_function(tableName,table_stats);
                                        client.query(query, cb);
                                    })
                                    .defer(function(cb) {
                                    ...
                                    .defer(function(cb) {
                                        if (new_task) {
                                            ...
                                            var details = [];
                                            details.push(tableName); //change the idtask, also it change the table
                                            details.push(data.source);
                                            details.push(data.owner);
                                            details.push(data.description);
                                            details.push(false) //status =false, change to avalible task
                                            details.push(data.id); // modifed id task
                                            var query = 'UPDATE task_details SET id=$1, source=$2, owner=$3, description=$4, status=$5 WHERE id=$6;';
                                            client.query(query, details, cb);
                                        }
                                    })
