...
module.exports = {
    ...
    track: function(client, table, time, attributes, callback) {
        ...
        console.log(table +'--'+attributes.key)
                                ...
                                queue(1)
                                    ...
                                    .defer(function(cb) {
                                        if (new_task) {
                                           ...
                                           // details.push(taskid); 
                                            details.push(data.source);
                                            details.push(tableName);//change the tableName
                                            details.push(data.owner);
                                            details.push(data.description);
                                            details.push(false) //status =false, change to avalible task
                                            details.push(data.id); // modifed id task

                                            var query = 'UPDATE task_details SET source=$1, tasktable=$2, owner=$3, description=$4, status=$5 WHERE id=$6;';
                                            client.query(query, details, cb);
                                        }
                                    })
