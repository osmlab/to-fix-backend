 ...
 * @param {String} opts.task - the id of the task the item belongs to
...
module.exports = function(opts) {
  return TaskItems.findOne({
    where: { id: opts.item, task_id: opts.task }
  })
    .then(function(data) {
      ...
      if (data !== null) {
        ...
        return data.update(updated).then(function(is) {
          return {
            was: was,
            is: is.toJSON()
          };
        ...
        return TaskItems.create(opts).then(function(is) {
          return {
            was: was,
            is: is.toJSON()
          };
    ...
    .then(function(change) {
      ...
      const was = {
        status: change.was.status,
        user: change.was.lockedBy
      };
      const is = {
        status: change.is.status,
        user: opts.user
      };
      return updateStats(opts.task, was, is).then(function() {
        return change.is;
      });
    });
};
